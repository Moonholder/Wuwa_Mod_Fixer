// src-core/src/collector.rs — migrated verbatim from src/collector.rs
// Only change: removed `error!` macro (use log::error instead, already in scope via #[macro_use])
...
pub fn parse_resouce_buffer_path(
    ...
    for section_cap in section_re.captures_iter(content) {
        let section_content = match section_cap.get(0) {
            Some(m) => m.as_str(),
            None => {
                log::warn!("Invalid section format");
                continue;
            }
        };
            ...
            .map(|m| m.as_str().trim())
        {
            Some(name) => name,
            None => {
                log::warn!("Missing filename in section: {}", section_header);
                continue;
            }
        };
...
fn extract_component_indices<'a>(
    ...
    for cap in COMPONENT_RE.captures_iter(content) {
        ...
        if min_offset != usize::MAX {
            component_indices.insert(component_index, (max_end_offset - min_offset, min_offset));
        }
    }
    component_indices
}
